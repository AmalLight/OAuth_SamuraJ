...
function client_init ()
{
    ...
    jqueryId_none ( [ '#Data' , '#Shell' ] , true ) ;

    global_functions.loadOldState () ;
    global_functions.setState ( 'group' , null ) ;
    
    loadDiscovery () ;
    
    if ( global_functions.getParam ( 'access_token' ) != null ) {

        jqueryId_none ( [ '#Data' , '#Shell' ] , false ) ;
        
        var payload = JSON.parse ( base64UrlDecode  ( global_functions.getParam ( 'access_token' ).split ( '.' ) [ 1 ] ) ) ;

        if ( 'group' in payload ) global_functions.setState ( 'group' , payload [ 'group' ] ) ;
    }
