...
def first_text  ( input_name ) : return '<text> {} </text>'.format ( input_name )
def second_text ( input_ads  ) : return '<text> {} </text>'.format ( input_ads  )
...
end_Names = """
  ...
  <div id=space></div>
  <div id=space></div>
""".                  format ( second_text ( "Machine's Name you are pointing for ." ) )
...
""".                    format (
    ...
    first_text  ( 'Actions From Selects' ) ,
    second_text ( 'You can Select Here additional Commands to execute on Machine you are pointing for .' )
)
